...
| **simple** | Bug with known cause, or a small change in one component/page | `brief` (one short doc: problem, assumptions, success criteria, task checklist) → gate → implement → validate-lite |
...
| **comprehensive** | Cross-cutting (routing/auth/cart state), risky, migration, unfamiliar area, or team review wanted | full + worktree isolation + full verification (TYPECHECK + LINT + BUILD + UNIT_TEST) mandatory per slice + design/structure published for team review before proceeding |
...
- Tier changes the ceremony, never the safety rails: verification verbs, the
  diff-ownership gate, and "no node_modules/dist modification, all API calls through src/services/api.ts" apply at every tier.
