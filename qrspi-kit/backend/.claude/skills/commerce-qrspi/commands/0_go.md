...
| **simple** | Bug with known cause, or a small change in one extension | `brief` (one short doc: problem, assumptions, success criteria, task checklist) → gate → implement → validate-lite |
...
| **comprehensive** | Cross-extension, risky, migration, unfamiliar area, or team review wanted | full + worktree isolation + INTEGRATION_TEST mandatory per slice + design/structure published for team review before proceeding |
...
- Tier changes the ceremony, never the safety rails: verification verbs, the
  diff-ownership gate, and "no OOTB/gensrc modification" apply at every tier.
