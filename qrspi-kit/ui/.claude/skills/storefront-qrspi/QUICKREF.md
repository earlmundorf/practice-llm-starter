...
(here: `npx tsc --noEmit`, `npm run lint`, `npm run build`; `UNIT_TEST` is `MANUAL:`
until a test runner lands — see `_notes`). Research layers also live in config
(`researchLayers`), which is what makes this skill framework-neutral. A verb may be
`MANUAL: <steps>` —
...
Combined repos (`core-customize/` + `js-storefront/`): do NOT install this variant
there — use commerce-qrspi alone with an `FE_*` verb namespace (see its SKILL.md).
...
never modify node_modules//dist · all backend calls through the API boundary layer ·
