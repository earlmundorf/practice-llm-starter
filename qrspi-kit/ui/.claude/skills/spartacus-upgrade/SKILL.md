---
name: spartacus-upgrade
description: |
  Upgrade a SAP Spartacus storefront from its current version to a target version,
  stepping through each required major version one at a time. Handles Angular
  version alignment, schematics execution, build verification, and common
  migration fixes at each step. Supports Spartacus 2.x through 2211.x,
  including minor/patch updates within the 2211.x line.
  Also trigger with: "spartacus upgrade", "upgrade spartacus", "spartacus migration",
  "update spartacus", "ng update spartacus", "spartacus version", "spartacus update".
argument-hint: "[targetVersion, e.g. 2211.32 or 6.8]"
allowed-tools: [Read, Grep, Glob, Edit, Write, Bash(ng *), Bash(npm *), Bash(npx *), Bash(node *), Bash(yarn *), Bash(pnpm *), Bash(git *)]
effort: high
---
