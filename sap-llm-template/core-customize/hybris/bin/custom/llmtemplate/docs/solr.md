# Solr Search Configuration — llmtemplate / ThinkShop

## Overview

The llmtemplate extension configures Solr faceted search for the 10 ThinkShop electronics products. This enables full-text search, price range faceting, stock status faceting, and multiple sort options via the OCC `/products/search` endpoint.
...
The Solr ImpEx (`essentialdata-llmtemplate_solr.impex`) loads automatically during initialize/updatesystem.
...
| File | Purpose |
|------|---------|
| `resources/impex/essentialdata-solr-llmtemplate.impex` | Solr config (auto-loaded on init/update) |
| `docs/solr.md` | This documentation |

The Solr ImpEx is named `essentialdata-solr-llmtemplate.impex` so it sorts alphabetically after
`essentialdata-llmtemplate.impex` (`s` > `m`), ensuring Currency and Catalog exist first.
