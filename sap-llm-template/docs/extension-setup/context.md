...
- **Code generation** — `ant build` generates Java classes from `*-beans.xml` (DTOs) and `*-items.xml` (Models). Generated code goes to `gensrc/` and should never be hand-edited.

## Key Conventions

- **HSQLDB for development** — File-based database, zero setup. Switch to MySQL/HANA via `local.properties` for production.
