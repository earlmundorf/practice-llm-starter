...
- **Code Generation** — The `ant build` process that generates Java classes from `items.xml` and `beans.xml` definitions
...
graph TD
    ...
    subgraph Code Generation ["ant build"]
...
- **Code generation** from `items.xml` requires `ant clean all` (full rebuild), while `beans.xml` changes only need `ant build`.
