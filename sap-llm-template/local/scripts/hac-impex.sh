...
if [ -z "$INPUT" ]; then
  ...
  echo "" >&2
  echo "Environment:" >&2
  echo "  HAC_URL=https://localhost:9002  HAC_USER=admin  HAC_PASS=nimda" >&2
  exit 1
fi
...
if 'data-level=\"notice\"' in content and 'successfull' in content:
    print('OK: ImpEx import successful')
    # Extract log text if present
...
if 'data-level=\"error\"' in content and 'unsuccessfull' in content:
    print('ERROR: ImpEx import failed', file=sys.stderr)
    # Extract unresolved data
    m = re.search(r'<div class=\"box impexResult quiet\">\s*<pre>\s*(.*?)\s*</pre>', content, re.DOTALL)
    if m:
