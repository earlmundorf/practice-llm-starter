...
description: |
  Reviews SAP Commerce (Hybris) code against best practices. Applies judgment about layer separation, Spring wiring, type system design, FlexibleSearch patterns, OCC conventions, and performance.

  Trigger this skill when the user asks to review code quality, check best practices, audit an extension, or asks "is this right?" about SAP Commerce code. Also trigger with: "review", "best practices", "code review", "audit", or "check my code".
context: fork
agent: Explore
...
### Controller — Thin HTTP Adapter with @Resource Injection
...
public class McpController
{
	...
	public String handlePost(
		...
		@RequestHeader(value = "MCP-Session-Id", required = false) final String sessionId,
		final HttpServletResponse response)
	{
		try
		{
			...
			{
				final McpDispatcherService.InitializeResult result =
					mcpDispatcherService.handleInitialize(request);
				response.setHeader("MCP-Session-Id", result.getSessionId());
			...
			final JsonRpcResponse jsonRpcResponse =
				mcpDispatcherService.dispatch(request, session);
...
Controller is a thin HTTP adapter — no business logic. `@Resource(name = ...)` for explicit bean injection. `@Secured` for role-based access. Delegates everything to the dispatcher service.

### Spring XML — Alias Pattern with List-Based Handler Registration

```xml
<!-- Session management -->
<alias name="defaultMcpSessionService" alias="mcpSessionService"/>
<bean id="defaultMcpSessionService"
      class="com.llmtemplate.services.impl.DefaultMcpSessionService"/>

<!-- Tool Handlers -->
<alias name="defaultProductSearchToolHandler" alias="productSearchToolHandler"/>
<bean id="defaultProductSearchToolHandler"
      class="com.llmtemplate.tools.impl.ProductSearchToolHandler">
...
Every bean uses the alias pattern: `defaultXxx` aliased to `xxx`. Other extensions can override by redefining the alias. List-based registration for pluggable tool handlers. Setter injection with `<property>` references platform beans by alias.

### Strategy Pattern — Tool Handler with Facade Delegation
...
public class ProductSearchToolHandler implements McpToolHandler
{
	...
	@Override
	public String getName() { return "product_search"; }
	...
	public Map<String, Object> getInputSchema()
	{
		...
		schema.put("properties", Map.of(
			...
			"pageSize", Map.of("type", "integer", "description", "Results per page", "default", 20)
		));
	...
	public McpToolResult execute(final Map<String, Object> args)
	{
		try
		{
			...
			final int pageSize = args.containsKey("pageSize")
				? ((Number) args.get("pageSize")).intValue() : 20;

			final SearchStateData searchState = new SearchStateData();
			final SearchQueryData queryData = new SearchQueryData();
			queryData.setValue(query + ":relevance");
...
Each tool handler delegates to an existing commerce facade — no new domain logic. `@Required` setter for Spring XML injection. Error handling returns a result, not an exception — the protocol layer decides how to present it.
