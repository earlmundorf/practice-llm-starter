...
- **Workflow findings** — the stages themselves could be better. A research layer missed
  ...
  every project, into the kit's canonical stage commands.
...
`.claude/skills/qrspi/`. That skill directory is installed by the QRSPI kit and replaced
wholesale on update; anything you wrote there would be lost. This directory is yours and
...
1. **Start of a ticket (stage 1):** the skill lists `working-docs/findings/*.md` (excluding
   README/TEMPLATE) and loads any whose `applies_to` matches the ticket's area/type — so
   research starts informed by what prior tickets learned.
...
3. **End of a ticket (stage 7):** summarize new findings and propose which to **promote**
   into the config, the repo's CLAUDE.md, or the kit's canonical stage commands. Promotion
   is always user-approved. Mark promoted findings `status: promoted`.
