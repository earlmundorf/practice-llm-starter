...
coremcp returns a single `text` content block containing JSON-serialized
facade data.
...
```
POST /occ/v2/{baseSiteId}/agent/visual-search
```

Accepts a base64-encoded image and returns matching catalog products.
Uses GPT-4o Vision for product identification, then runs a 3-tier Solr catalog search.

**This is a standalone OCC endpoint, not part of the MCP protocol.**
It does not require an MCP session — just an OAuth token.
...
| `mimeType` | string | no | MIME type (default: `image/jpeg`). Allowed: `image/jpeg`, `image/png`, `image/webp`, `image/gif` |

### Response (200 OK)

```json
{
  "visionAnalysis": "A pair of black wireless over-ear headphones with cushioned ear cups",
  "aiDetail": {
    "searchTerms": "black wireless headphones",
    "reasoning": "The image shows over-ear headphones with a matte black finish and cushioned ear cups, consistent with premium noise-canceling models.",
    "confidence": 0.9,
  ...
  "products": [
    {
      "product": {
        ...
        "code": "WH1000XM5-BLK",
        "price": { "value": 349.99, "formattedValue": "$349.99", "currencyIso": "USD" },
        "stock": { "stockLevelStatus": "inStock", "stockLevel": 15 },
        "images": [
          { "url": "/medias/...", "format": "thumbnail" },
          { "url": "/medias/...", "format": "product" }
        ],
        "categories": [
...
| Field | Description |
|-------|-------------|
| `visionAnalysis` | One-sentence description of what GPT-4o sees in the image |
| `aiDetail` | AI reasoning details: `searchTerms`, `reasoning`, `confidence`, `productName`, `brand`, `category`, `color` |
| `products` | Array of matched products in full OCC shape, ordered by confidence |
| `products[].matchType` | `bestMatch` (brand+name match, label "Best Match"), `similar` (attribute match), `explore` (category fallback, label "You Might Like") |
| `products[].confidence` | 0.95 or 0.9 (bestMatch — 0.9 when using searchTerms fallback), 0.7 (similar), 0.4 (explore) |
