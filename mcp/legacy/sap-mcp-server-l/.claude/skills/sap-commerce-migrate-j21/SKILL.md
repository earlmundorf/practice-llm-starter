...
| **Data matches** | Row counts for 5–10 critical types within tolerance (Product, Customer, Order, Catalog, ContentPage at minimum) **AND** orphaned-types cleanup completed without removing intended data **AND** stale-cart/order deserialization sample passes (see [`references/known-incidents.md`](./references/known-incidents.md) #6) |
...
This skill runs a SAP Commerce Cloud migration to the **2211-jdk21.x update line** (first release September 2025; monthly cadence since — 2211-jdk21.10 as of May 2026): Java 21, Spring 6.2.10+, Tomcat 10.1, Jakarta EE 10. Target the latest patch at migration time. **Deadline context:** no new Java-17 CCv2 builds after **2026-08-31** — factor the runway into every plan.

For version pins, validation history, and changelog, see [[`CHANGELOG.md`](./CHANGELOG.md)](./CHANGELOG.md) next to this file.

## Before anything else

1. Read [`references/00-overview.md`](./references/00-overview.md). Three minutes. It anchors version targets and headline changes so the rest of this file makes sense.
2. List `findings/*.md` (excluding `README.md` and [`TEMPLATE.md`](./findings/TEMPLATE.md)). Each finding represents something a prior run learned that isn't yet in the authoritative references. Unpromoted findings still apply — load any whose `applies_to:` frontmatter matches the target project's state.
...
3. **Capture intake.** Copy [`references/intake-template.md`](./references/intake-template.md) → `migration-docs/intake.md`. Auto-populate the "Auto-detected state" table from Step 2's output. Then ASK the user how they want to fill in the remaining fields:
...
5. **Walk the decision tree.** Read [`references/decision-tree.md`](./references/decision-tree.md). For each branch that applies to the detected state (per intake fields), note which `references/sap-docs/*.md` file(s) the plan should cite. Strategy choice (intake 2.1) and Phase B path (intake 2.2) come directly from intake — no separate decision needed here.
6. **Consult authoritative refs as needed.** `references/sap-docs/` holds Markdown mirrors of the SAP Help Portal pages (fetched via the reverse-engineered JSON API — see script comment in [`scripts/fetch_sap_docs.sh`](./scripts/fetch_sap_docs.sh)). These are authoritative. The full list:
   - [`01-general-update-guide.md`](./references/sap-docs/01-general-update-guide.md) — overall update guide, including the fully-manual-update anchor and the OpenRewrite anchor
   - [`02-openrewrite-recipes.md`](./references/sap-docs/02-openrewrite-recipes.md) — pointer to the OpenRewrite section of `01`
   - [`03-spring-6.md`](./references/sap-docs/03-spring-6.md) — Spring 6 landing page (TOC of sub-topics)
   - [`04-spring-model-changes.md`](./references/sap-docs/04-spring-model-changes.md) — exhaustive bean/model diff
   - [`05-tomcat.md`](./references/sap-docs/05-tomcat.md) — Tomcat 10 changes landing
   - [`06-build-ant-gradle.md`](./references/sap-docs/06-build-ant-gradle.md) — Ant/Gradle changes
   - [`07-testing.md`](./references/sap-docs/07-testing.md) — test framework changes
   - [`08-oauth-authorization-server.md`](./references/sap-docs/08-oauth-authorization-server.md) — Spring Authorization Server (the new OAuth)
   - [`09-resource-server.md`](./references/sap-docs/09-resource-server.md) — resource server config (OCC side)
   - [`10-resttemplate-removal.md`](./references/sap-docs/10-resttemplate-removal.md) — RestTemplate/OAuth2RestTemplate deprecation
   - [`11-smartedit.md`](./references/sap-docs/11-smartedit.md) — SmartEdit adjustments
   - [`12-web-service-exception-handling.md`](./references/sap-docs/12-web-service-exception-handling.md) — error handler changes
   - [`13-release-notes-2211-jdk21.md`](./references/sap-docs/13-release-notes-2211-jdk21.md) — release notes stream for the 2211-jdk21 line
   - [`14-update-release-2211.46.md`](./references/sap-docs/14-update-release-2211.46.md) — specific point-release notes

   Also authoritative: [`references/additional-changes.md`](./references/additional-changes.md) — items verbatim from SAP's framework-update .docx that don't have their own dedicated SAP Help page. Covers detailed Spring 6 sub-topics, Apache libraries, caching, JVM/language housekeeping, Olingo, DdlUtils, orphaned types, LocalizedHybrisConstraintViolation, and yacceleratorstorefront-specific jakarta steps. Treat this file as peer-authoritative to `sap-docs/`.

   Curated companion (not a SAP mirror): [`references/oauth-migration-guide.md`](./references/oauth-migration-guide.md) — the ordered make-it-work path for the new Spring Authorization Server OAuth, the 8 client-validation preconditions, a validated `OAuthClientDetails` cookbook, and SAML SSO impact. Load it during Phase D planning alongside `sap-docs/08-oauth-authorization-server.md`.

   Raw upstream source the mirrors were derived from, in [`references/upstream/`](./references/upstream/README.md): the Spring framework upgrade notes for [6.0](./references/upstream/SPRING_UPGRADES_60.md), [6.1](./references/upstream/SPRING_UPGRADES_61.md) and [6.2](./references/upstream/SPRING_UPGRADES_62.md). Go here only when a `sap-docs/` page is ambiguous and you need the primary Spring wording — these are upstream Spring docs, not SAP guidance, so they say nothing about Commerce specifics.
7. **Write the plan.** Start from [`references/phase-guide.md`](./references/phase-guide.md). Write to the target project's `migration-docs/migration-plan.md`. **Substitute all `{{...}}` placeholders from `migration-docs/intake.md`** — see the substitution map at the bottom of the intake template (e.g., `{{IN_PLACE | COPY}}` from intake 2.1, `{{LEGACY_PROJECT_DIR}}` from 3.2, `{{TARGET_COMMERCE_VERSION}}` from 1.1). Phases run A→H as in the guide; skip any that don't apply to the detected state. For Phase 0, copy ONLY the subsection matching the chosen strategy (`Phase 0-inplace` or `Phase 0-copy`); drop the other. **For Phase 0-prep Steps 0.0a/b**: tailor based on intake 4.1 (persistence scenario) — see the conditional branches inline in [`phase-guide.md`](./references/phase-guide.md). Every step must link to a specific `references/sap-docs/*.md` (and, if relevant, to [`references/known-incidents.md`](./references/known-incidents.md) entries).
8. **Record supporting-skill findings.** Write `migration-docs/supporting-skill-findings.md` in the target project alongside the plan. This captures what the skill wants to learn FROM this run — e.g., "I didn't know whether extension X uses OAuth2RestTemplate; confirm during Phase D and update [`decision-tree.md`](./references/decision-tree.md) Branch 3".
...
OpenRewrite itself remains a documented alternative in [`references/sap-docs/02-openrewrite-recipes.md`](./references/sap-docs/02-openrewrite-recipes.md) + [`references/sap-notes/3618495-openrewrite-framework-update.md`](./references/sap-notes/3618495-openrewrite-framework-update.md) — use it for codebases with 5+ custom extensions where grep-per-pattern volume is unmanageable. For the ≤5-extension common case, Claude sweeps are faster to set up and easier to review.

Steps:

1. **Confirm the approved plan exists** at `migration-docs/migration-plan.md`.
2. **Walk Phase B's residue catalog** (see [`references/phase-guide.md`](./references/phase-guide.md) Phase B). For each sweep:
...
- **During the run:** when the skill hits something that isn't in the authoritative refs, write a `findings/YYYY-MM-DD-{slug}.md` entry using [`findings/TEMPLATE.md`](./findings/TEMPLATE.md). Even tiny findings count.
...
The SAP Help Portal is a Vue SPA. [`scripts/fetch_sap_docs.sh`](./scripts/fetch_sap_docs.sh) mirrors the pages listed in [`scripts/upgrade_resources.md`](./scripts/upgrade_resources.md) (next to the script) as Markdown into `references/sap-docs/` using the reverse-engineered `/http.svc/deliverableMetadata` + `/http.svc/pagecontent` JSON API. Re-run any time SAP updates docs:
...
- Some pages (e.g., [`03-spring-6.md`](./references/sap-docs/03-spring-6.md), [`05-tomcat.md`](./references/sap-docs/05-tomcat.md)) are landing pages linking to sub-topics with their own LOIOs. The mirror captures the landing. If a sub-topic is needed verbatim, add its LOIO to the `LOIO_FILE` map in the script and re-run.
...
- Verifying migration outcomes against the gates in [`references/verification-checklist.md`](./references/verification-checklist.md)
