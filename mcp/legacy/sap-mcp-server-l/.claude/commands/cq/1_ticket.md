...
2. Write `ticket.md`: problem statement in your own words, business context, draft
   ...
   acceptance criteria, explicit out-of-scope list. Record the agreed tier.
...
4. Cover every category in `questionCategories` from `working-docs/config.json` — or, when
   that is `null`, every layer name in `researchLayers` — explicitly where relevant, plus
   configuration and build/env concerns.
