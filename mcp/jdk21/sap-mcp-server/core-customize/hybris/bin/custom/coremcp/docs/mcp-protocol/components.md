...
| `src/com/coremcp/services/McpSessionService.java` | Interface: `createSession(clientInfo, protocolVersion)`, `getSession(sessionId)`, `removeSession(sessionId)`. |
| `src/com/coremcp/services/impl/DefaultMcpSessionService.java` | In-memory implementation using `ConcurrentHashMap<String, McpSession>`. `createSession` generates `sess_` + 12-char truncated UUID. `getSession` calls `touch()` on access to update `lastAccessedAt`. `removeSession` deletes from the map. |
