...
Tests run via direct ant, **not** Gradle. The `sap.commerce.build` Gradle plugin (5.0.2) silently ignores `-Dtestclasses.extensions=` and runs the full ~11000-test platform suite regardless. Direct ant honors the filter and runs only the requested extensions.

```bash
./scripts/junit-tenant-init.sh                       # ONCE: create junit_* tenant schema
...
**Re-run `junit-tenant-init.sh`** any time `*-items.xml` changes (after `yall + yupdatesystem` against master, the junit tenant needs the same type-system update). Or run `cd hybris/bin/platform && source ./setantenv.sh && ant yunitupdate` for an in-place update without full re-init.

**Reports:** per-class XML at `core-customize/hybris/temp/hybris/junit/TEST-*.xml`; HTML summary at `core-customize/hybris/log/junit/index.html`.
