...
export const auth = {
  login: async (email: string, password: string): Promise<void> => {
    ...
    // Spring Authorization Server (Spring Security 6) requires confidential
    // clients to authenticate via HTTP Basic; body-form client credentials
    // are rejected before grant_type is parsed.
    const response = await fetch('/authorizationserver/oauth/token', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Authorization: `Basic ${btoa('trusted_client:secret')}`,
      },
      body: body.toString(),
    });
